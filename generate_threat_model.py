...
for resultado in semgrep_data.get("results", []):
    ...
    try:
        if "SQL Injection" in mensagem:
            ameaca_sqli = Threat(SID=f"SQLI-{contador_global}")
            ...
            ameaca_xss = Threat(SID=f"XSS-{contador_global}")
            ...
            ameaca_mass_assignment = Threat(SID=f"MA-{contador_global}")
            ...
            ameaca_improper_validation = Threat(SID=f"IV-{contador_global}")
            ...
            ameaca_code_injection = Threat(SID=f"CI-{contador_global}")
            ...
            ameaca_debug_code = Threat(SID=f"ADC-{contador_global}")
